...
Vagrant.configure("2") do |config|
  ...
  settings['hosts'].each do |my_host|
	...
	if ($is_salt_master == true)
	  ...
	  salt.seed_master = {
		             "loadbalancer.example.com" => "saltstack/keys/loadbalancer.example.com.pub",
		             "web01.example.com" => "saltstack/keys/web01.example.com.pub",
		             "web02.example.com" => "saltstack/keys/web02.example.com.pub"
			     }		
	...
	if ($is_lb_minion == true)
	  salt.minion_config = "saltstack/etc/loadbalancer"
          salt.minion_key = "saltstack/keys/loadbalancer.example.com.pem"
          salt.minion_pub = "saltstack/keys/loadbalancer.example.com.pub"
	  salt.run_highstate = false
	end
	if ($is_web01_minion == true)
	  salt.minion_config = "saltstack/etc/web01"
          salt.minion_key = "saltstack/keys/web01.example.com.pem"
          salt.minion_pub = "saltstack/keys/web01.example.com.pub"
	  salt.run_highstate = false
	end
	if ($is_web02_minion == true)
	  salt.minion_config = "saltstack/etc/web02"
          salt.minion_key = "saltstack/keys/web02.example.com.pem"
          salt.minion_pub = "saltstack/keys/web02.example.com.pub"
	  salt.run_highstate = false
	end
      end   
    end
